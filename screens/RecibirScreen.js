...
const RecibirScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const obtenerTransferencia = async () => {
      try {
        const response = await fetch('http://192.168.1.108:5000/api/transferencia/verificar', {
          method: 'POST',
          headers: {
  ...
  const actualizarEstadoTransferencia = async (transferenciaId) => {
    try {
      const response = await fetch('http://192.168.1.108:5000/api/transferencia/actualizar', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
